...
class Bacola_Product_Grid2_Widget extends Widget_Base {
    use Bacola_Helper;

    public function get_name() {
        return 'bacola-product-grid2';
    }
	...
	protected function _register_controls() {
       ...
       $this->add_control( 'title',
		...
		$defaultimage = plugins_url( 'images/product-grid2.jpg', __DIR__ );
        ...
        $this->add_control( 'image_second_subtitle',
            [
        ...
        $this->add_control( 'image_btn_link',
            [
                'label' => esc_html__( 'Image Link', 'bacola-core' ),
                'type' => Controls_Manager::URL,
                'label_block' => true,
                'placeholder' => esc_html__( 'Place URL here', 'bacola-core' )
            ]
	...
	protected function render() {
		...
		$imagetarget = $settings['image_btn_link']['is_external'] ? ' target="_blank"' : '';
		$imagenofollow = $settings['image_btn_link']['nofollow'] ? ' rel="nofollow"' : '';
	...
	query_posts( $args );
		...
		<?php
			...
			$output .= '<div class="site-module module-categor-products style-1">';
		...
		if( have_posts() ) : while ( have_posts() ) : the_post();
			...
			$output .= '<div class="products column-2 mobile-column-2 align-center">';
			}
			...
			if($count == $leftcolumn){
				...
				$output .= '<div class="column center">';
				...
				$output .= '<div class="category-text color-danger">'.esc_html($settings['image_title']).'</div>';
				$output .= '</div><!-- content-header -->';
				$output .= '<div class="content-main">';
				$output .= '<h4 class="entry-subtitle color-text xlight">'.esc_html($settings['image_subtitle']).'</h4>';
				$output .= '<h3 class="entry-title color-text-light">'.esc_html($settings['image_second_subtitle']).'</h3>';
				$output .= '<div class="entry-store color-info-dark">'.esc_html($settings['image_desc']).'</div>';
				...
				$output .= '<a href="'.esc_url($settings['image_btn_link']['url']).'" '.esc_attr($imagetarget.$imagenofollow).' class="overlay-link"></a>';
